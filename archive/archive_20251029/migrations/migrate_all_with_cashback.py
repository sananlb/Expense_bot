#!/usr/bin/env python
"""
ПОЛНАЯ МИГРАЦИЯ ВСЕХ ДАННЫХ ИЗ SQLITE В POSTGRESQL (ВКЛЮЧАЯ КЕШБЕКИ)
"""
import sqlite3
...
print("=" * 50)
print("ПОЛНАЯ МИГРАЦИЯ ВСЕХ ДАННЫХ (С КЕШБЕКАМИ)")
print("=" * 50)
...
cursor.execute("SELECT COUNT(*) FROM expenses_cashback")
total_cashbacks = cursor.fetchone()[0]
...
print(f"  Кешбеков: {total_cashbacks}")

# Импортируем модели Django
from expenses.models import Profile, Household, ExpenseCategory, Expense, RecurringPayment, UserSettings, Cashback

# ОЧИСТКА СУЩЕСТВУЮЩИХ ДАННЫХ
print("\n1. Очищаем существующие данные в PostgreSQL...")
Cashback.objects.all().delete()
...
for row in expenses:
    ...
    expense = Expense.objects.create(
        ...
        cashback_excluded=bool(row['cashback_excluded']) if row['cashback_excluded'] else False,
        cashback_amount=Decimal(str(row['cashback_amount'])) if row['cashback_amount'] else Decimal('0'),
    )
    migrated += 1
...
# МИГРАЦИЯ КЕШБЕКОВ
print("\n7. Мигрируем кешбеки...")
cursor.execute("SELECT * FROM expenses_cashback")
...
for row in cashbacks:
    ...
    cashback = Cashback.objects.create(
        ...
        cashback_percent=Decimal(str(row['cashback_percent'])),
        month=row['month'],
        limit_amount=Decimal(str(row['limit_amount'])) if row['limit_amount'] else None,
        description=row['description'] or '',
        category=category,
        created_at=datetime.fromisoformat(row['created_at']) if row['created_at'] else datetime.now(),
        updated_at=datetime.fromisoformat(row['updated_at']) if row['updated_at'] else datetime.now(),
    ...
    cat_name = category.name if category else "Все категории"
    print(f"  + Кешбек {row['bank_name']}: {row['cashback_percent']}% для {cat_name}")

print(f"  Итого кешбеков: {cashback_count}")

# МИГРАЦИЯ РЕГУЛЯРНЫХ ПЛАТЕЖЕЙ
print("\n8. Мигрируем регулярные платежи...")
...
print(f"  Кешбеков: {Cashback.objects.count()}")
...
try:
    ...
    cashbacks = Cashback.objects.filter(profile=profile)
    ...
    print(f"  Кешбеков: {cashbacks.count()}")
    ...
    if cashbacks.exists():
        print(f"\n  Кешбеки:")
        for cb in cashbacks:
            cat_name = cb.category.name if cb.category else "Все категории"
            print(f"    {cb.bank_name}: {cb.cashback_percent}% для {cat_name}")
