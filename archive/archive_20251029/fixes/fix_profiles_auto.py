...
if duplicates:
    for d in duplicates:
        ...
        for p in profiles:
            ...
            # Запоминаем профиль с данными
            if expense_count > 0 or category_count > 0:
                main_profile = p
                print(f'    -> Это основной профиль')

        # Удаляем пустые дубликаты
        if main_profile:
            for p in profiles:
                if p.id != main_profile.id:
                    expense_count = Expense.objects.filter(profile=p).count()
                    category_count = ExpenseCategory.objects.filter(profile=p).count()
                    if expense_count == 0 and category_count == 0:
                        print(f'  Удаляем пустой дубликат ID={p.id}')
                        p.delete()
else:
    print('Дубликатов не найдено')

print('\n=== УДАЛЕНИЕ ПУСТЫХ ПРОФИЛЕЙ (если нет данных) ===')
empty_profiles = []
for p in Profile.objects.all():
    ...
    if expense_count == 0 and category_count == 0:
        ...
        print(f'Найден пустой профиль: id={p.id}, telegram_id={p.telegram_id}')

# Удаляем только профили без данных
if empty_profiles:
    print(f'\nУдаляем {len(empty_profiles)} пустых профилей...')
    for p in empty_profiles:
        print(f'  Удален профиль id={p.id}, telegram_id={p.telegram_id}')
        p.delete()
else:
...
for p in Profile.objects.all().order_by('id'):
    ...
    if expense_count > 0:
        ...
        print(f'  ID={p.id}, telegram_id={p.telegram_id}:')
        print(f'    - {expense_count} расходов на {total:.2f} руб')
