...
## 🚨 КРИТИЧЕСКИ ВАЖНО: АВТОЦИКЛ CODEX REVIEW ДЛЯ ПЛАНОВ 🚨

### Мультимодельный workflow: Claude Code + Codex

**Документация:** `docs/VIBE_CODING_WORKFLOW.md`

При Edit/Write любого файла `*plan*.md` автоматически запускается Codex CLI для ревью.
Codex думает 1-4 минуты (хук синхронный, Edit/Write не вернётся пока Codex не закончит).
Результат сохраняется в `.codex-review-result.md`.

### ОБЯЗАТЕЛЬНЫЙ ЦИКЛ после каждого Edit/Write план-файла:

```
1. Edit/Write *plan*.md (ожидание 1-4 мин — Codex ревьюит)
2. Read .codex-review-result.md — прочитать замечания Codex
3. Если Status: APPROVED → цикл завершён, сообщить пользователю
...
7. Повторять пока Codex не скажет APPROVED
```

### ЗАПРЕЩЕНО:
- Игнорировать вывод Codex
- Пропускать чтение `.codex-review-result.md` после Edit/Write план-файла
- Считать план готовым без APPROVED от Codex
- Отклонять замечания без аргументации

### Файлы системы:
- `.claude/hooks/codex-review-hook.sh` — PostToolUse хук (ревью планов)
- `.claude/hooks/codex-code-review-hook.sh` — Stop хук (ревью git diff)
- `.claude/hooks/codex-session-cleanup.sh` — SessionStart (очистка)
- `.codex-review-result.md` — результат последнего ревью (читать после Edit/Write)
