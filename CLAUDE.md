...
## 🧭 ПРИНЦИП РАБОТЫ: ДЕЛЕГИРОВАНИЕ И КООРДИНАЦИЯ

**Claude выступает координатором.** По умолчанию поручай отдельные подзадачи
субагентам (Task), где это можно сделать без вреда для общей эффективности, а сам
сосредоточься на декомпозиции задачи, постановке заданий, проверке и сборке
результатов.

### Когда делегировать субагентам
- **Исследование и поиск** в коде → `general-purpose` / `Explore` (поиск функций,
  мест использования, понимание архитектуры перед изменениями).
- **Проектирование плана** сложной фичи → `Plan`.
- **Параллельные независимые правки** в разных файлах → несколько субагентов
  одновременно в одном вызове (см. раздел про параллельный запуск ниже).
- **Обновление индекса документации** → `docs-indexer`.

### Когда НЕ делегировать (делать самому)
- Тривиальные правки, опечатки, мелкие однофайловые изменения — быстрее сделать напрямую.
- Финальная сборка, согласование интерфейсов между файлами, разрешение
  противоречий между результатами субагентов — это работа координатора.
- Когда делегирование добавит больше накладных расходов, чем сэкономит
  (каждый субагент стартует «с нуля» и заново собирает контекст).

### Codex — это тоже субагент (для сложных и проверочных задач)
Используй Codex (через файл `.codex-task`, см. раздел ниже) как отдельного
субагента для:
- ревью готовых планов перед реализацией;
- ревью git diff / сложных изменений (миграции, потокобезопасность, биллинг);
- получения второго мнения по неочевидным архитектурным решениям;
- фокусированных проверочных вопросов.

**Правило по результатам субагентов и Codex:** каждое замечание явно прими или
аргументированно отклони, покажи пользователю краткую сводку решений. Итоговую
ответственность за корректность несёт координатор, а не субагент.
