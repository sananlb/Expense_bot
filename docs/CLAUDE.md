...
**ВСЕГДА копируем проект ПОЛНОСТЬЮ при обновлении сервера:**
1. НЕ копируем отдельные файлы выборочно
2. НЕ вносим изменения напрямую на сервере
3. Структура директорий на сервере ДОЛЖНА точно соответствовать репозиторию
4. ЗАПРЕЩЕНО копировать отдельные файлы (Dockerfile, requirements.txt и т.д.)
