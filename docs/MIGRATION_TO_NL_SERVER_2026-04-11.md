# Переезд проекта на нидерландский сервер

**Дата составления:** 2026-04-10  
**Дата завершения:** 2026-04-11 ✅  
**Старый сервер:** 176.124.218.53 (Россия, Ubuntu 24.04.3) — остановлен  
**Новый сервер:** `144.31.97.139` (Нидерланды) — активен  
...
| **Провайдер** | NuxtCloud |
| **Тариф** | NL-RZ9-1 |
| **Домен** | vm1265930.cloud.nuxt.network |
| **ОС** | Ubuntu 22.04.2 LTS |
| **RAM** | 4 GB |
| **Диск** | 59 GB |
| **Расположение** | Нидерланды |
| **Панель управления** | https://vm.nuxt.cloud |
| **Пользователь** | batman / Aa07900790 |
| **SSH** | `ssh batman@144.31.97.139` |

### Что сделано на новом сервере

- ✅ Создан пользователь `batman` (пароль Aa07900790, sudo без пароля)
- ✅ SSH ключи добавлены в `authorized_keys`
- ✅ Root доступ по SSH отключён (`PermitRootLogin no`)
- ✅ Docker 29.4.0 установлен, batman добавлен в группу docker
- ✅ docker-compose-plugin установлен (синтаксис `docker compose`)
- ✅ nginx, certbot, ufw установлены
- ✅ Репозиторий склонирован в `/home/batman/expense_bot`
- ✅ `.env` настроен (без прокси, `BOT_MODE=webhook`)
- ✅ БД перенесена: 3023 траты, 358 пользователей
- ✅ DNS обновлён на DuckDNS: expensebot → 144.31.97.139
- ✅ SSL сертификат Let's Encrypt получен (действует до 2026-07-09)
- ✅ Nginx настроен с HTTPS, конфиг из репозитория
- ✅ Все 6 Docker контейнеров запущены
- ✅ Webhook зарегистрирован: `https://expensebot.duckdns.org/webhook/`
- ✅ Автообновление SSL через certbot.timer

### Что убрано по сравнению со старым сервером

- ❌ `TELEGRAM_API_HOST_OVERRIDE` — не нужен (нет блокировок из NL)
- ❌ `TELEGRAM_FORCE_IPV4` — не нужен
- ❌ `AI_PROXY_URL` — не нужен
- ❌ `OPENROUTER_CONNECTION_MODE=proxy` → теперь `direct`
- ❌ `httpx-socks`, `aiohttp-socks` — удалены из requirements.txt
- ❌ Весь proxy-код из `unified_ai_service.py` и `telegram_client.py`

### Дополнительно оптимизировано

- ✅ Dockerfile разбит на multi-stage: `base` (без Playwright) + `playwright`
- ✅ `celery`, `celery-beat`, `web` — собираются из `base` (быстро, ~3 мин)
- ✅ `bot` — собирается из `playwright` (с Chromium для PDF)
- ✅ Время пересборки при обновлениях сократилось с ~30 мин до ~3-5 мин

### Старый сервер (176.124.218.53)

Контейнеры `bot`, `celery`, `celery-beat`, `web` остановлены.  
БД и Redis оставлены запущенными до полного подтверждения стабильности.  
Сервер можно отключить через 1-2 дня после подтверждения что всё работает.
