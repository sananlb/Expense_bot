...
- ✅ Обработка реферальных ссылок в /start
- ✅ Начисление комиссий при оплате
- ✅ Команда /affiliate для пользователей
- ✅ Роутер добавлен в систему
- 🔄 Тестирование
- ⏳ API интеграция с Telegram для выплат

### Следующие шаги
1. Celery задача для обработки холда комиссий (21 день)
2. Уведомления рефереру о новых платежах
3. Админ-команды управления программой
4. Интеграция с Telegram API для автоматических выплат звёзд
5. Дополнительное тестирование на реальных платежах
...
1. **Получение реферальной ссылки:**
   - Отправьте боту команду `/affiliate`
   - Скопируйте вашу уникальную ссылку

2. **Регистрация по реферальной ссылке:**
   - Новый пользователь должен перейти по вашей ссылке
   - Бот автоматически зарегистрирует реферальную связь

3. **Начисление комиссий:**
   - Когда ваш реферал оплатит подписку (150 или 600 звёзд)
   - Вы получите 10% комиссию (15 или 60 звёзд)
   - Комиссия будет на холде 21 день
   - После холда звёзды будут начислены

4. **Просмотр статистики:**
   - `/affiliate` - основная информация
   - Кнопки в сообщении для детальной статистики

### Для разработчиков

```bash
# Запуск тестов локально
./venv/Scripts/python.exe test_affiliate_integration.py

# Проверка миграций
./venv/Scripts/python.exe manage.py showmigrations expenses

# Проверка в Django shell
./venv/Scripts/python.exe manage.py shell
>>> from expenses.models import AffiliateProgram
>>> AffiliateProgram.objects.all()
```
