...
def match_keyword_in_text(keyword: str, text: str) -> Tuple[bool, str]:
    ...
    if len(keyword_words) == 1 and len(text_words) >= 1:
        if len(normalized_keyword) >= 4 and len(text_words[0]) >= 4:
            ...
            if normalized_keyword[:stem_len] == text_words[0][:stem_len]:
...
| `"зарплата"` | `"Зарплату перевели"` | ✅ | inflection | Одиночный keyword + первое слово в склонении |
| `"зарплата"` | `"Зарплата от компании"` | ✅ | inflection | Одиночный keyword + первое слово точно |
| `"тест"` | `"Сосиска в тесте и чай"` | ❌ | none | "тест" != первое слово → НЕТ совпадения! |
...
2. **Inflection только для первого слова текста** — "тест" НЕ матчит "сосиска в тесте"
...
### Production bug (ноябрь 2025) и решение

**Найденная проблема:** "Сосиска в тесте и чай 390" → категория "Родственники" (из-за keyword "тест")
...
**Результат:**
- ✅ Нет ложных срабатываний ("тест" не матчит "в тесте")
- ✅ Склонения работают ("зарплата" → "зарплату")
- ✅ Умный prefix matching для фраз
