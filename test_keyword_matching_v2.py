...
def test_no_false_positives():
    ...
    test_cases = [
        ...
        # УДАЛЕНО: ("тест", "Сосиска в тесте и чай", False) - теперь ДОЛЖНО совпадать через inflection (ANY word)!
        # УДАЛЕНО: ("водица", "Водица на тренировку", False) - это ДОЛЖНО совпадать через inflection!
...
def test_inflections():
    ...
    test_cases = [
        # (keyword, text, should_match, expected_type)
        # ВАЖНО: склонения работают для одиночного keyword с ЛЮБЫМ словом текста
        # ВАЖНО: diff <= 1 (только 1 символ разницы для предотвращения ложных срабатываний)

        # Одиночное слово vs одиночное слово (разница 1 символ)
        ("зарплата", "Зарплату", True, "inflection"),  # зарплата → зарплату (а → у, diff=1)
        ("зарплата", "Зарплаты", True, "inflection"),  # зарплата → зарплаты (а → ы, diff=1)
        ("зарплата", "Зарплате", True, "inflection"),  # зарплата → зарплате (а → е, diff=1)

        # Одиночное слово vs фраза - матчит первое слово
        ("зарплата", "Зарплату перевели", True, "inflection"),
        ("зарплата", "Зарплата от компании", True, "inflection"),
        ("водица", "Водица на тренировку", True, "inflection"),

        # НОВОЕ: Одиночное слово vs фраза - матчит ЛЮБОЕ слово (не только первое)
        ("зарплата", "мне перевели зарплату", True, "inflection"),
        ("зарплата", "перевели зарплату вчера", True, "inflection"),
        ("доход", "получил доходы сегодня", True, "inflection"),

        # НЕ должны срабатывать - разница > 1 символа (СТРОГАЯ ПРОВЕРКА)
        ("фриланс", "Фрилансом", False, "none"),  # разница 2 символа (фриланс → фрилансом)
        ("кофе", "Кофейня", False, "none"),  # разница > 1 символа
        ("тест", "тестирование", False, "none"),  # разница > 1 символа

        # НЕ должны срабатывать - короткие слова или числа
...
def test_real_world_scenarios():
    ...
    test_cases = [
        # Сценарий 1: "Сосиска в тесте и чай 390" - теперь "тест" матчит "тесте" (inflection ANY word)
        ("тест", "Сосиска в тесте и чай 390", True),  # НОВОЕ: inflection матчит ЛЮБОЕ слово

        # Сценарий 2: "Водица" как одиночный keyword ДОЛЖЕН матчить фразу через inflection
        ("водица", "Водица на тренировку 130", True),  # ДОЛЖНО совпадать через inflection

        # Сценарий 3: Правильное совпадение
        ("сосиска в тесте и чай", "Сосиска в тесте и чай 390", True),  # prefix совпадение
        ("сосиска в тесте и чай", "Сосиска в тесте 400", True),  # prefix совпадение

        # Сценарий 4: Долг за тест
        ("долг за тест", "Долг за тест 500", True),  # prefix совпадение
