    ...
    def generate_xlsx_with_charts(
        ...
        if income_category_stats:
            ...
            if sorted_income_categories_list and sorted_days:
                ...
                day_cell.font = header_font
                day_cell.fill = PatternFill(start_color="7FB685", end_color="7FB685", fill_type="solid")  # Мятный зеленый
                day_cell.alignment = header_alignment
                day_cell.border = thin_border
                ...
                # Размещение столбчатой диаграммы доходов (справа от круговой, смещение +8 колонок)
                # Используем +8 (аналогично расходам где +7, но круговая доходов чуть шире: 19.76 vs 19.3)
                income_bar_col = get_column_letter(income_summary_start_col + 8)
