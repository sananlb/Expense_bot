...
1. get_max_expense_day(period='last_week'|'last_month'|'week'|'month') - для вопросов "В какой день я больше всего потратил на прошлой неделе?"
...
4. get_min_single_expense(period='last_week'|'last_month'|'week'|'month') - для "Какая моя самая маленькая/дешёвая трата на прошлой неделе/в прошлом месяце?"
5. get_category_statistics() - для "На что я трачу больше всего?"
6. get_average_expenses() - для "Сколько я трачу в среднем?"
7. get_recent_expenses(limit=10) - для "Покажи последние траты"
8. search_expenses(query='текст', period='last_week'|'last_month') - УНИВЕРСАЛЬНЫЙ ПОИСК по категориям И описаниям: "Сколько потратил на продукты на прошлой неделе?", "Траты на кофе в прошлом месяце"
9. get_weekday_statistics() - для "В какие дни недели я трачу больше?"
10. predict_month_expense() - для "Сколько я потрачу в этом месяце?"
11. check_budget_status(budget_amount=50000) - для "Уложусь ли я в бюджет?"
12. compare_periods() - для "Я стал тратить больше или меньше?"
13. get_expense_trend() - для "Покажи динамику трат"
14. get_expenses_by_amount_range(min_amount=1000) - для "Покажи траты больше 1000"
15. get_category_total(category='продукты', period='month'|'week'|'январь'|...|'декабрь'|'зима'|'лето') - для КАТЕГОРИЙ: "Сколько я потратил на продукты в этом месяце/в августе/летом?"
...
20. get_max_single_income(period='last_week'|'last_month'|'week'|'month'|'январь'|'февраль'|...|'декабрь'|'january'|'august'|'зима'|'весна'|'лето'|'осень') - для "Какой мой самый большой доход на прошлой неделе/в прошлом месяце/в августе/летом?"
...
36. analytics_query(spec_json='<JSON-спецификация запроса>') - для нестандартных аналитических запросов
