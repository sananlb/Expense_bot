...
# ВАЖНО: "+" считается маркером дохода когда стоит перед суммой (числом).
# При этом "+" не должен быть частью слова (C++, G+) — перед ним должен быть
# пробел или начало строки, а после — цифра (с опциональным пробелом).
INCOME_PATTERNS = [
    r'^\+\s*\d',  # Начало строки + цифра: +5000, + 5000
    r'(?<=\s)\+\s*\d',  # Пробел перед +, затем цифра: долг +1200, кофе + 200
