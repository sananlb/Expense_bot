...
AMOUNT_PATTERNS = [
    ...
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:{_RU_RUB}|rub+les?|руб|₽|р)\b',
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:{_RU_USD}|dollars?|долл|usd|\$)\b',
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:euros?|{_RU_EUR}|eur|€)\b',
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:pounds?|{_RU_GBP}|gbp|£)\b',
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:{_RU_CNY}|yuan|cny|¥)\b',
    ...
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:бразильских?|reais?|{_RU_BRL}|brl)\b',
    # CIS currencies
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:{_RU_GEL}|lari|gel|georgian\s+lari)\b',
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:{_RU_KZT}|тнг|kzt|tenge|kazakh\S*\s+tenge)\b',
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:{_RU_UAH}|грн|uah|hryvnia|hryvnya|ukrainian\s+hryvnia)\b',
    rf"(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:{_RU_UZS}|so['\']m|uzs|uzbek\S*\s+so['\']m)\b",
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:{_RU_AMD}|dram|amd|armenian\s+dram)\b',
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:{_RU_AZN}|manat|azn|azerbaijani\s+manat)\b',
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:{_RU_KGS}|kgs|kyrgyz\S*\s+som)\b',
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:{_RU_TJS}|somoni|tjs|tajik\S*\s+somoni)\b',
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:{_RU_MDL}|mdl|lei|moldovan\s+lei?)\b',
    r'(\d{1,3}(?:[\s,]\d{3})+(?:[.,]\d+)?)\s*(?:бел[ао]рус\S*\s+руб\S*|byn|byr|belarusian\s+ruble?)\b',
    # Other world currencies
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:{_RU_CHF}|francs?|chf|swiss\s+francs?)\b',
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:{_RU_INR}|rupees?|inr|indian\s+rupees?)\b',
    rf'(\d{{1,3}}(?:[\s,]\d{{3}})+(?:[.,]\d+)?)\s*(?:{_RU_TRY}|liras?|try|turkish\s+liras?)\b',
    r'(\d{1,3}(?:[\s,]\d{3})+(?:[.,]\d+)?)\s*$',
    r'^(\d{1,3}(?:[\s,]\d{3})+(?:[.,]\d+)?)\s',
    # Fallback
...
# Правило: "+" перед числом = доход. "+" должен быть на границе слова
