...
AMOUNT_PATTERNS = [
    ...
    r'(\d{1,3}(?:\.\d{3})+)\s*(?:рублей|рубля|рубль|rub+les?|руб|₽|р)\b',
    ...
    r'(\d{1,3}(?:[\s,]\d{3})+(?:[.,]\d+)?)\s*(?:рублей|рубля|рубль|rub+les?|руб|₽|р)\b',
    ...
    r'^(\d{1,3}(?:[\s,]\d{3})+(?:[.,]\d+)?)\s',
    # Fallback
    r'(\d+(?:[.,]\d+)?)\s*(?:рублей|рубля|рубль|rub+les?|руб|₽|р)\b',
...
# Паттерны для определения дохода - знак "+", слово "плюс" или "plus"
# Единое правило: маркер считается доходом когда стоит перед суммой (числом).
# Маркер должен быть на границе слова (начало строки или после пробела).
# После маркера — цифра (с опциональным пробелом).
INCOME_PATTERNS = [
    r'^\+\s*\d',  # +5000, + 5000
    r'(?<=\s)\+\s*\d',  # долг +1200, кофе + 200
    r'^плюс\s*\d',  # плюс 5000, плюс5000
    r'(?<=\s)плюс\s*\d',  # зарплата плюс 3000, зп плюс5000
    r'^plus\s*\d',  # plus 5000, plus5000
    r'(?<=\s)plus\s*\d',  # bonus plus 1200, salary plus5000
]
