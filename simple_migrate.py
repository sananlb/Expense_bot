    ...
    try:
        ...
        tg_user, created = TelegramUser.objects.update_or_create(
            ...
            defaults={
                ...
                'is_premium': bool(row['is_premium']) if row['is_premium'] else False,
                'trial_used': bool(row['trial_used']) if row['trial_used'] else False,
...
for tg_user in TelegramUser.objects.all():
    expenses = Expense.objects.filter(user=tg_user).count()
    categories = Category.objects.filter(user=tg_user).count()
    recurring = RecurringPayment.objects.filter(user=tg_user).count()
    print(f"  {tg_user.username or tg_user.telegram_id}: {expenses} расходов, {categories} категорий, {recurring} регулярных")
